<?php
$per ->h(1,500,170,'اضافة مستخدم جديد');
$per ->f0('index.php?uc=NPER2','post');
$per ->submit(1110,525,'اضافة مستخدم جديد');
$per ->fieldset("field1","Donnees de l'état civil");
$x=250;
$per ->label(925,$x,'*اللقب');                 $per ->txtar(720,$x,'NOMAR',20,'');
$per ->label(650,$x,'*الاسم');                  $per ->txtar(470,$x,'PRENOMAR',20,'');
...
$per ->label(623-20,$x3,'المنح العائلية');         
$per ->label(580-20,$x3,'نعم');                    $per ->radioed(560-20,$x3,"ALLF","1");    
$per ->label(540-20,$x3,' لا ');                    $per ->radio(520-20,$x3,"ALLF","0");  
$per ->label(430-70,$x3,'عمل الزوجة');               
$per ->label(400-70,$x3,'نعم');                    $per ->radio(380-70,$x3,"CONJOINT","1");    
$per ->label(360-70,$x3,' لا ');                    $per ->radioed(340-70,$x3,"CONJOINT","0");  
$per ->label(120,$x3,'*عدد الاولاد');            $per ->nbr(35,$x3,'NBRE','10',0,10); 
...
$x5=445;
$per ->label(840,$x5,'*المستوى الدراسي');      $per ->combo(650,$x5,'NE','grh','niveauetude','المستوى الدراسي','data','0','2'); 
$per ->label(500,$x5,'*المؤهل');               //$per ->combo(500,$x5,'NE','grh','niveauetude','المؤهل','data','0','1'); 
...
$per ->fieldset("field5","****");
$per ->label(1170,$x,'* NSS رقم الضمان الاجتماعيى ');  $per ->txt(1005,$x,'NSS',15,'');
$per ->label(1230,$x1,'* CCP رقم ح/ب');               $per ->txt(1005,$x1,'CCP',15,'');
$per ->label(1220,$x2,'*رقم الهاتف المحمول ');           $per ->txt(1005,$x2,'TELP',15,'');
$per ->label(1230,$x3,'*رقم الهاتف الثابت');           $per ->txt(1005,$x3,'TELF',15,'');
