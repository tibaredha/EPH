...
{
header("Location: ../index.php?uc=DECESDATEBE") ;
}
...
$h=7;
$pdf->SetXY(0.5,$h); 	  
$pdf->cell(1,0.5,"Age",1,0,'C',1,0);
$pdf->SetXY(1.5,$h); 	  
$pdf->cell(1,0.5,"Sexe",1,0,'C',1,0);
$pdf->SetXY(2.5,$h); 	  
$pdf->cell(5,0.5,"Commune Residence",1,0,'C',1,0);
$pdf->SetXY(7.5,$h); 	  
$pdf->cell(4,0.5,"Service Hosp",1,0,'C',1,0);
$pdf->SetXY(11.5,$h); 	  
$pdf->cell(2,0.5,"Durée hosp ",1,0,'C',1,0);
$pdf->SetXY(13.5,$h); 	  
$pdf->cell(7,0.5,"Cause Du Décés",1,0,'C',1,0);
...
while($row=mysql_fetch_object($resultat))
  {
    $pdf->cell(1,1,$row->AGE,1,0,'C',0);
    $pdf->cell(1,1,$row->SEX,1,0,'C',0);
    $pdf->cell(5,1,$row->COMMUNEDER,1,0,'L',0); //$pdf->nbrtostring("grh","com","IDCOM",$row->COMMUNEDER,"COMMUNE")probleme avec  ancienne base qui sous forme string 
    $pdf->cell(4,1,$row->SERVICEDHO,1,0,'L',0);
	$pdf->cell(2,1,$row->DUREEHOSPI,1,0,'C',0);
	$pdf->cell(7,1,$row->CAUSEDUDEC,1,0,'L',0);
    $pdf->SetXY(0.5,$pdf->GetY()+1); 
  }
$pdf->SetXY(0.5,$pdf->GetY()); 	  
$pdf->cell(6,0.5,"TOTAL",1,0,'C',1,0);	  
$pdf->SetXY(6.5,$pdf->GetY()); 	  
$pdf->cell(14,0.5,$totalmbr1." Décés",1,0,'C',1,0);				 
