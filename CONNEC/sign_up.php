			...
			if(preg_match('#^(([a-z0-9!\#$%&\\\'*+/=?^_`{|}~-]+\.?)*[a-z0-9!\#$%&\\\'*+/=?^_`{|}~-]+)@(([a-z0-9-_]+\.?)*[a-z0-9-_]+)\.[a-z]{2,}$#i',$_POST['email']))
			{
				...
				if($dn==0)
				{
					...
					if(mysql_query('insert into users(id, username, password, email, avatar, signup_date) values ('.$id.', "'.$username.'", "'.$password.'", "'.$email.'", "'.$avatar.'", "'.time().'")'))
					{
                       ...
                       echo "<a href=\"connexion.php\">Se connecter</a></div>";						
...
echo " Veuillez remplir ce formulaire pour vous inscrire:<br />";	
echo " <div class=\"center\">";	
			...
			echo "\" /><br />";
			echo "<label for=\"password\">Mot de passe<span class=\"small\">(6 caract&egrave;res min.)</span></label> ";
			echo "<input type=\"password\" name=\"password\" /><br />";
			echo "<label for=\"passverif\">Mot de passe<span class=\"small\">(v&eacute;rification)</span></label> ";
			echo "<input type=\"password\" name=\"passverif\" /><br />";
			...
			echo "\" /><br />";
			echo "<label for=\"avatar\">Image perso<span class=\"small\">(facultatif)</span></label> ";
